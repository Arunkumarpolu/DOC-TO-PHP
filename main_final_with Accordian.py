...
def wrap_ul_with_diseases_div(html_content):
    """Wrap all <ul> elements in a div with class 'diseases' and clean ul classes"""
    soup = BeautifulSoup(html_content, 'html.parser')
...
def generate_php_file(template_path, output_path, sections):
    ...
    main_content = wrap_ul_with_diseases_div(main_content)
...
def insert_cta_in_content(content_html, cta_top, cta_bottom):
    """
    Inserts the CTA include snippets into the content HTML:
      - Top CTA after ~30% of eligible paragraphs.
      - Bottom CTA after ~80% of eligible paragraphs.
    """
    soup = BeautifulSoup(content_html, "html.parser")
    ...
    if total:
        index_top = min(int(total * 0.3), total - 1)
        index_bottom = min(int(total * 0.8), total - 1)
        paragraphs[index_top].insert_after(BeautifulSoup(cta_top, "html.parser"))
        paragraphs = [p for p in soup.find_all("p") if not p.find_parent("table")]
        paragraphs[index_bottom].insert_after(BeautifulSoup(cta_bottom, "html.parser"))
    else:
        soup.append(BeautifulSoup(cta_top, "html.parser"))
        soup.append(BeautifulSoup(cta_bottom, "html.parser"))
